...
//            return ChatStateDAO.new(this.context) {
    ...
    fun toModel(): NewChatState? = transaction {
        return@transaction when {
            ...
            else -> ExternalChatState(
                chatId,
                state!!.toStateId(),
                StateConnectionsDAO.find { StatesConnectionsTable.initialStateId eq state }.map { it.toModel() },
                description = StateQuestionsDAO.find { StatesQuestionsTable.id eq state }.first().description
            )
        }
    }
}